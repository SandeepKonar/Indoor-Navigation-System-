...
class CreatePath  : AppCompatActivity(), View.OnClickListener, ARcoreDataProvider.arDataListener,
    ARcoreMinimalDataProvider.arDataListener {
...
//    var stopButton : FloatingActionButton? = null
    ...
    override fun onClick(v: View?) {
        if (v?.id==R.id.startButton) {
            ...
            var arCoreDataProvider = ARcoreMinimalDataProvider(this,arFragment,rootRelatievLayout.width,rootRelatievLayout.height)
...
//                                      "z" to "$z"))
        count+=1
        var text = findViewById<TextView>(R.id.createPathInfo)
        text.text="x: ${-z} y: $x"

        Toast.makeText(this, "NODE $count ADDED to path", Toast.LENGTH_SHORT).show()
        //Please stand at the exact place where you wish the navigation should start and end (ideally in the middle of doors of expected places)
