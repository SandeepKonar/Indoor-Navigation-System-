...
r_image = cv2.resize(image,None,fx=2,fy=2,interpolation=cv2.INTER_AREA)
rgb = cv2.cvtColor(r_image, cv2.COLOR_BGR2RGB)
...
for i in range(0, len(results["text"])):
	...
	if conf > args["min_conf"]:
		...
		cv2.putText(r_image, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX,
			1.2, (0, 0, 255), 3)
